      ...
      try {
        ...
        await resend.emails.send({
          ...
          html: `
            <!DOCTYPE html>
            <html>
            <body style="font-family:sans-serif;background:#f9f5f0;padding:24px;margin:0">
              <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e5ddd5">
                <div style="background:linear-gradient(135deg,#2d4a2d,#4a7c59);padding:28px 32px">
                  ...
                  <p style="color:rgba(255,255,255,0.7);margin:6px 0 0;font-size:13px">
                    Diterima: ${new Date().toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'short' })}
                  </p>
                </div>
        ...
        await resend.emails.send({
          from: 'Sarau Luxury <noreply@sarau-luxury.com>',
          to: [body.email],
          subject: `✅ ${isBooking ? 'Inquiry' : 'Pesan'} Anda telah kami terima – Sarau Luxury`,
          html: `
            <!DOCTYPE html>
            <html>
            <body style="font-family:sans-serif;background:#f9f5f0;padding:24px;margin:0">
              <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e5ddd5">
                ...
                <div style="padding:32px">
                  ...
                  <p style="color:#4a3728;line-height:1.6">
                    Terima kasih telah menghubungi <strong>Sarau Luxury</strong>.
                    ${isBooking
                      ? 'Inquiry booking Anda telah berhasil kami terima.'
                      : 'Pesan Anda telah berhasil kami terima.'
                    }
                  </p>
                  <p style="color:#4a3728;line-height:1.6">
                    Tim kami akan menghubungi Anda melalui <strong>WhatsApp atau email</strong> dalam
                    <strong>1×24 jam kerja</strong>.
                  </p>
                  ${isBooking ? `<p style="color:#4a3728;line-height:1.6">Nomor referensi Anda: <strong style="color:#2d4a2d">SL-${Date.now().toString().slice(-6)}</strong></p>` : ''}
